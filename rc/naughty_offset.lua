...
----------------------------------------------------------------------------
-- @editor ahuszagh@gmail.com
-- patched to allow exact offsets by passing a position_exact parameter
----------------------------------------------------------------------------
...
local function process_offset(screen, position, position_exact, idx, width, height)
    ...
    -- calculate x
    v.x = position_exact.x - 50
    v.y = position_exact.y - 100
...
local function arrange(screen)
    for p,pos in pairs(naughty.notifications[screen]) do
        for i,notification in pairs(naughty.notifications[screen][p]) do
            local offset = process_offset(screen, p, notification.position_exact, i, notification.width, notification.height)
...
function naughty.notify(args)
    ...
    local position_exact = args.position_exact
    ...
    notification.position_exact = position_exact
    ...
    -- position the wibox
    local offset = process_offset(screen, notification.position, notification.position_exact, nil, notification.width, notification.height)
